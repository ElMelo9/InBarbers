...
class UbicacionRepository:
    ...
    def getById(self, id: int) -> Optional[dict]:
        response = self.client.from_("ubicaciones").select(
            "*").eq("id_ubicacion", id).execute()
        return response.data[0] if response.data else None
    ...
    def getUltUbiByUsuario(self, id: int) -> Optional[dict]:
        response = (
            self.client
            .from_("ubicaciones")
            .select("*")
            .eq("id_usuario", id)
            # Ordenar por fecha_rg en orden descendente
            .order("fecha_rg", desc=True)
            .limit(1)  # Limitar el resultado a solo 1 registro
            .execute()
    )
        return response.data[0] if response.data else None
    ...
    def update(self, id: int, data: dict) -> Optional[dict]:
        response = self.client.from_("ubicaciones").update(
            data).eq("id_ubicacion", id).execute()
    ...
    def delete(self, id: int):
        ...
        response = self.client.from_("ubicaciones").delete().eq(
            "id_ubicacion", id).execute()
